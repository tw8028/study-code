...
( 
	...
	fn fn_export oldDir bHavePrefix = 
	(
		arrExport = #()
		...
		(
			...
			(
				...
				--saveMaxFile  ( strBipedPath + "\\" + (filterstring 3 ".")[1] + "_biped.max" ) clearNeedSaveFlag:true useNewFile:false quiet:true
				...
				--skinMesh = undefined
				--for a in objects where (classof a == PolyMeshObject) do ( skinMesh = a )
				for a in objects where ((classof a == PolyMeshObject or classof a ==  Editable_Mesh  ) and  
											( findstring a.name "mor" ) == undefined ) do ( appendifunique objlist a)	
				arrExport = objlist
				--appendifunique arrExport skinMesh	
				hide ( for a in objects where (( finditem arrExport a ) == 0 ) collect a )
				...
				select arrExport
				Filename = strFBXPath  + "\\" + (filterstring (strPrefix + (substring maxfilename nIndex 100)) ".")[1] + ".fbx"
	...
	(--filein (getdir #maxroot+"scripts\\TLM_Assets_Inspector.ms")
						...
						( 
							...
							(	
							 ...
							 (
								 ...
								 print "found objects with same name in scene :"
								 print newName
							 )
						 )
		...
		(
			...
			group  "Output" 
			(
				dropdownlist ddl_paths   width:125 height:20 items:#(GetDir #export) tooltip:"" across:2
				button btn_selectPath "..." width:25 height:20 offset:[-25,0]
				checkbox chbx_haveprefix checked:false offset:[165,-24]
				label lb_prefix "¼ÓÇ°×ºf"  pos:[203, 190]
				label lb_hitbone "Chk Hitbone"  pos:[14,220] --align:#righ
				...
				(
					...
					(
						...
						(
							...
							--fn_bakeAnimation();
							( 
								fn_export str_exportPath chbx_haveprefix.checked
								--fn_resetScene()
							)
							else
								(
		...
		rollout RL_checker "Check Scene" width:245 height:40
		(
			group "Check Tools"
			(
				button btn_checkObj "Check \"Obj\" Objects" height:16 width:110  align:#left apos:[10,120]
				button btn_checkParticle "Delete Particle" height:16 width:110  align:#right apos:[10,100]	offset:[0,-21]	
			)
			
			
			on btn_checkObj pressed do
			(
				objModel = #()
				objModel = for a in objects where (((classof a) == Editable_Poly or ( classof a ) == Editable_mesh ) and ((findstring a.name "object") != undefined )) collect a
				if ( objModel.count > 0) then
				(
					RL_Main_CR.theSubRollout.RL_Output.dnc_label2.text = "WARNING : \"object\" model detectd!\n"
				)
				else
				( RL_Main_CR.theSubRollout.RL_Output.dnc_label2.text = "GOOD!No \"object\" model in scene:)\n" )
			)
			
			on btn_checkParticle pressed do
			(
				 arr_PView = for a in objects where (classof a) == Particle_View collect a
				 if (arr_PView.count > 0) then
				 (
					 --print ((arr_PView as string) + " is deleted.\n");
					 delete arr_PView
					 RL_Main_CR.theSubRollout.RL_Output.dnc_label2.text = "Particle is deleted.\n"
					 
				  ) 
				  else
				  (
					   RL_Main_CR.theSubRollout.RL_Output.dnc_label2.text = "There is no particle in scene."
				  )
				  --else (append str_correctLog  "There is no Particle View in this Scene.\n" )
			)
		)
		
		
		rollout RL_Output "Message" width:245 height:120
		(
