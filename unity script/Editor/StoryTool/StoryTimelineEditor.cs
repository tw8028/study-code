...
namespace StoryTool
{
	public class StoryTimelineEditor : EditorWindow
	{
		...
		public void CreateGUI()
		{
			...
			btnExportExcel.RegisterCallback<ClickEvent>(evt => TimelineInfo.MsgToExcel(CurrentTimeline));

			Button btnImportExcel = new() { text = "Import from excel" };
			btnImportExcel.RegisterCallback<ClickEvent>(evt => TimelineInfo.ImportFromExcel(CurrentTimeline));
			...
			btnTool4.RegisterCallback<ClickEvent> (evt => TimelineInfo.InsertBlock(CurrentTimeline));
