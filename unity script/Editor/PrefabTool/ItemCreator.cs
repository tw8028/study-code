...
namespace PersonBrowser
{
    public class ItemCreator : EditorWindow
    {
        ...
        {
            Button createGunButton = new() { name = "button1", text = "选择 fbx 创建 gun prefab" };
            rootVisualElement.Add(createGunButton);
            createGunButton.RegisterCallback<ClickEvent>(CreateGun);

            Button createBagButton = new() { name = "button2", text = "选择 fbx 创建 bag prefab" };
        ...
        public void CreateBag(ClickEvent evt)
        {
            ...
            foreach (GameObject obj in objectArray)
            {
                ...
                {
                    Debug.LogWarning("不是背包，请选择 bag fbx");
                    return;
                }
        ...
        public void CreateGun(ClickEvent evt)
        {
            ...
            foreach (GameObject obj in objectArray)
            {
                ...
                {
                    Debug.LogWarning("不是武器，请选择 gun fbx");
                    return;
                }
