...
class IkSystem(IJoint_limb, ABC):
    def __init__(self, cog: CenterOfGravity):
        ...
        self._ik_handle = f'ikHandle__{self.side}__{self.name}__001'
        self.ctrl_ikHandle = f'ctrl__{self.side}__{self.name}_ikHandle__001'
        self._zero_ikHandle = f'zero__{self.side}__{self.name}_ikHandle__001'
        self._ctrl_pole = f'ctrl__{self.side}__{self.name}_pole__001'
        self._zero_pole = f'zero__{self.side}__{self.name}_pole__001'
    ...
    def _rig(self):
        ...
        pm.parent(self._zero_ikHandle, self.grp_rig)
        ...
        pm.parent(self._zero_pole, self.grp_rig)
        pm.poleVectorConstraint(self._ctrl_pole, self._ik_handle)
        pm.parent(mytools.connect_line(self._ctrl_pole, self.joints_ik[1]), self.grp_rig)
