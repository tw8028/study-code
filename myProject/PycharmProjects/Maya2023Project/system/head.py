...
class Head:
    ...
    def __init__(self, *, joints: list[str]):
        self.joints = joints
        self.root = f'root__c__neck__001'
    ...
    def create(self):
        # create fk joints
        mytools.jnt_target(name=self.root, target=self.joints[0])
        for jnt, jnt_fk in zip(self.joints, self.joints_fk):
            mytools.jnt_target(name=jnt_fk, target=jnt)
        mytools.parent_chain([self.root] + self.joints_fk)
    ...
    def rig(self):
        ...
        root_nd = pm.PyNode(self.root)
        name = self.root.split('__', 1)[1]
        ...
        pm.orientConstraint(self.ctrl_head, self.head_fk)
